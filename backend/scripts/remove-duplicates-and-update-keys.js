...
console.log('🧹 Removing duplicate songs and updating keys...\n');

// Găsește toate melodiile duplicate (păstrează ultima înregistrare pentru fiecare titlu)
const duplicates = db.prepare(`
  SELECT s1.id, s1.title, s1.created_at
  FROM songs s1
  WHERE EXISTS (
    SELECT 1 FROM songs s2
    WHERE s2.title = s1.title AND s2.id > s1.id
  )
  ORDER BY s1.title, s1.id
`).all();

console.log(`📋 Found ${duplicates.length} duplicate songs to remove\n`);

// Șterge duplicatele
for (const song of duplicates) {
  console.log(`🗑️  Deleting duplicate: "${song.title}" (ID: ${song.id})`);
  db.prepare('DELETE FROM songs WHERE id = ?').run(song.id);
}

console.log(`\n✅ Removed ${duplicates.length} duplicate songs\n`);

// Verifică câte melodii au rămas
const remainingSongs = db.prepare('SELECT COUNT(*) as count FROM songs').get();
console.log(`📊 Remaining songs: ${remainingSongs.count}\n`);

// Acum adaugă tonalitățile la toate melodiile
const songsWithKeys = [
  ...
  { title: 'Hai Deschide Inima Ta', keys: ['G'] },
  ...
  { title: 'Știind Că-i Viu (Because He Lives)', keys: ['C', 'D'] },
...
console.log('🎵 Updating keys for all songs...\n');
...
for (const songData of songsWithKeys) {
  ...
  if (!song) {
    console.log(`❌ Not found: "${songData.title}"`);
    notFound++;
    continue;
  }
  ...
  if (songData.keys.length === 0) {
    console.log(`⚠️  "${song.title}" - No keys specified`);
    updated++;
    continue;
  }
  ...
  console.log(`✅ "${song.title}" - ${songData.keys.length} keys: ${songData.keys.join(', ')}`);
  updated++;
}
...
console.log(`   ❌ Songs not found: ${notFound}`);
console.log(`   🎹 Total keys added: ${totalKeys}`);
console.log(`   📝 Total songs in database: ${remainingSongs.count}`);
